...
func TestClient_DemoteToBackup(t *testing.T) {
	// Provider p1 always returns 430, p2 always returns 223.
	// With threshold=3, p1 should be demoted after 3 missing responses.
	makeFactory := func(statusCode int) ConnFactory {
		return func(ctx context.Context) (net.Conn, error) {
			client, server := net.Pipe()
			go func() {
	...
	for range 10 {
		...
		// All requests should eventually succeed (p2 returns 223, or backup after demotion).
		if resp.StatusCode != 223 {
			t.Fatalf("StatusCode = %d, want 223", resp.StatusCode)
		}
	}
	...
	for _, ps := range stats.Providers {
		if ps.Demoted {
			demotedCount++
			if ps.Name != "bad" {
				t.Errorf("wrong provider demoted: %s", ps.Name)
			}
			if ps.Missing < threshold {
				t.Errorf("demoted provider Missing = %d, want >= %d", ps.Missing, threshold)
			}
		}
	}
	if demotedCount != 1 {
		t.Errorf("expected 1 demoted provider, got %d", demotedCount)
	}

	// Verify only 1 main remains.
	mains := *c.mainGroups.Load()
	if len(mains) != 1 {
		t.Errorf("expected 1 main provider, got %d", len(mains))
	}
	if mains[0].name != "good" {
		t.Errorf("remaining main = %s, want good", mains[0].name)
	}
}

func TestClient_DemoteProtectsLastMain(t *testing.T) {
	// Single main provider that always returns 430.
	// It must never be demoted (always keep at least one main).
	makeFactory430 := func() ConnFactory {
		return func(ctx context.Context) (net.Conn, error) {
			client, server := net.Pipe()
			go func() {
	...
	for range 10 {
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		<-c.Send(ctx, []byte("STAT <id@test>\r\n"), nil)
		cancel()
	}

	// Provider must not be demoted.
	mains := *c.mainGroups.Load()
	if len(mains) != 1 {
		t.Fatalf("expected 1 main provider, got %d", len(mains))
	}
	if mains[0].demoted.Load() {
		t.Error("sole main provider should not be demoted")
	}

	stats := c.Stats()
	for _, ps := range stats.Providers {
		if ps.Demoted {
			t.Errorf("provider %s should not be demoted", ps.Name)
		}
	}
}
